...
LOAD_MODEL = False
SAVE_MODEL = True
CHECKPOINT_GENERATOR_X = "C:/Users/mateo/OneDrive/Escritorio/[Año 4] 2do CUATRIMESTRE/Inteligencia Computacional/TP FINAL/modelos/genx90N.pth"
CHECKPOINT_GENERATOR_Y = "C:/Users/mateo/OneDrive/Escritorio/[Año 4] 2do CUATRIMESTRE/Inteligencia Computacional/TP FINAL/modelos/geny90N.pth"
CHECKPOINT_DISCRIMINATOR_X = "C:/Users/mateo/OneDrive/Escritorio/[Año 4] 2do CUATRIMESTRE/Inteligencia Computacional/TP FINAL/modelos/discx90N.pth"
CHECKPOINT_DISCRIMINATOR_Y = "C:/Users/mateo/OneDrive/Escritorio/[Año 4] 2do CUATRIMESTRE/Inteligencia Computacional/TP FINAL/modelos/discy90N.pth"
...
# ---------------------------------------------Preparar los datos-------------------------------------------------
archivo1 = "C:/Users/mateo/OneDrive/Escritorio/[Año 4] 2do CUATRIMESTRE/Inteligencia Computacional/TP FINAL/coeficientes90Mnorm.txt"
archivo2 = "C:/Users/mateo/OneDrive/Escritorio/[Año 4] 2do CUATRIMESTRE/Inteligencia Computacional/TP FINAL/coeficientes90Mnorm.txt"
...
train_labels = torch.cat(
    (torch.zeros(sizeDatosM), torch.ones(sizeDatosT))
)  # 0: Mateo, 1: Tobi

X_train, X_test, y_train, y_test = train_test_split(
    dataset, train_labels, test_size=0.1
)  # 80% para entrenamiento
X_val, X_test, y_val, y_test = train_test_split(
    X_test, y_test, test_size=0.5
)  # 10% validación y 10% testeo
...
val_loader = DataLoader(
    val_set, batch_size=batch_size, shuffle=True, drop_last=True, pin_memory=True
)
...
# torch.optim.SGD
optimizer_discriminatorX = torch.optim.Adam(
    Dx.parameters(), lr=learning_rate * 0.1
)  # *0.1
optimizer_discriminatorY = torch.optim.Adam(
    Dy.parameters(), lr=learning_rate * 0.1
)  # *0.1
...
epocaCorte = 0
if LOAD_MODEL:
    load_checkpoint(genX, CHECKPOINT_GENERATOR_X)
    load_checkpoint(genY, CHECKPOINT_GENERATOR_Y)
    load_checkpoint(Dx, CHECKPOINT_DISCRIMINATOR_X)
    load_checkpoint(Dy, CHECKPOINT_DISCRIMINATOR_Y)
# ---------------------------------------------Entrenamiento-------------------------------------------------
for epoch in tqdm(epochs):  # Para cada época
    for indiceBatch, (samples, labels) in enumerate(train_loader):  # Para cada batch
        ...
        # with torch.cuda.amp.autocast():#casteo mixto (para usar diferentes tipos de datos para ahorrar memoria)
        # Discriminador X-----------------------------------
        fakeX = genX(Y)
        ...
        Dx_real_loss = mse(
            Dx_real, torch.ones_like(Dx_real)
        )  # SMOOTHING LABELS -0.1*torch.ones_like(Dx_real)
        Dx_fake_loss = mse(
            Dx_fake, torch.zeros_like(Dx_fake)
        )  # +0.1*torch.ones_like(Dx_fake)
        ...
        Dy_fake = Dy(
            fakeY.detach()
        )  # Separa fakeY de la GPU (para no usarla para calcular gradientes)
        ...
        # discScaler.scale(D_loss).backward()
        # discScaler.step(optimizer_discriminatorX)
        # discScaler.step(optimizer_discriminatorY)
        # discScaler.update()
        ...
        identity_Y = genY(Y)
        identity_X = genX(X)
        identity_Y_loss = L1(Y, identity_Y)  # cambié por L1
        ...
        # genScaler.scale(G_loss).backward()
        # genScaler.step(optimizer_generatorX)
        # genScaler.step(optimizer_generatorY)
        # genScaler.update()
