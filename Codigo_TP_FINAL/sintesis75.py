...
audioM,sr = lb.load(path, sr=16000)
audioT,sr = lb.load(path2, sr=16000)
sampleRate = sr
cepOrder = 49
framePeriod = 80 #probar con 40 y 200 de frameLength
sizeFFT = 1024*8
frameLength = 800 
bitrate = sampleRate*16
# hopLength = framePeriod
# winLength = frameLength
hopLength = framePeriod
winLength = frameLength
normalizar = True
if normalizar == True:
    audioM = (1-1/2**bitrate)/max(audioM)*audioM
    audioT = (1-1/2**bitrate)/max(audioT)*audioT

#--------------------------------------Obtener coeficientes de mel-------------------------------------------------
#MATEO
melCoefM = lb.feature.mfcc(y=audioM,sr=sampleRate,n_mfcc=cepOrder,n_fft=sizeFFT,hop_length=hopLength, win_length=winLength)
melCoefM = melCoefM.T 

lowOrderMelM = melCoefM[:,:25]
highOrderMelM = melCoefM[:,25:]
...
#TOBIPE08
melCoefT = lb.feature.mfcc(y=audioT,sr=sampleRate,n_mfcc=cepOrder,n_fft=sizeFFT,hop_length=hopLength, win_length=winLength)
melCoefT = melCoefT.T

lowOrderMelT = melCoefT[:,:25]
highOrderMelT = melCoefT[:,25:]
# print(highOrderMelT.shape)
...
checkpoint_file1 = "C:/Users/mateo/OneDrive/Escritorio/[Año 4] 2do CUATRIMESTRE/Inteligencia Computacional/TP FINAL/modelos/geny75.pth"
checkpoint_file2 = "C:/Users/mateo/OneDrive/Escritorio/[Año 4] 2do CUATRIMESTRE/Inteligencia Computacional/TP FINAL/modelos/genx75.pth"
genX = Generator()
genY = Generator()
...
melCoefTobiasFake = np.concatenate((fake_tobias[:,:25],highOrderMelM),axis=1).T
melCoefMateoFake = np.concatenate((fake_mateo[:,:25],highOrderMelT),axis=1).T
#---------------------------------------------Síntesis-------------------------------------------------
print("Sintetizando=======>")
fakeT = lb.feature.inverse.mfcc_to_audio(mfcc=melCoefTobiasFake,n_mels=cepOrder,sr=sampleRate,hop_length=hopLength,win_length=winLength)#length= modifica el tamaño de salida de y 
# fakeT = (1-1/2**bitrate)/max(fakeT)*fakeT #normalización del audio
sf.write("C:/Users/mateo/OneDrive/Escritorio/[Año 4] 2do CUATRIMESTRE/Inteligencia Computacional/TP FINAL/Resultados/NuevosResultadosT.wav", fakeT, sampleRate)
# print("Pasó uno")
fakeM = lb.feature.inverse.mfcc_to_audio(mfcc=melCoefMateoFake,n_mels=cepOrder,sr=sampleRate,hop_length=hopLength,win_length=winLength)#length= modifica el tamaño de salida de y 
# fakeM = (1-1/2**bitrate)/max(fakeM)*fakeM
sf.write("C:/Users/mateo/OneDrive/Escritorio/[Año 4] 2do CUATRIMESTRE/Inteligencia Computacional/TP FINAL/Resultados/NuevosResultadosM.wav", fakeM, sampleRate)
...
print("MCD Tobias: ", MCDTobias) 
print("MCD Mateo: ", MCDMateo) #
#--------------------------------------Gráficas espectrograma-------------------------------------------------
if graficas == True:
